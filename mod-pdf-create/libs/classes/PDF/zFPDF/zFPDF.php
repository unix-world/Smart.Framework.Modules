...
class zFPDF
	...
	final public function AddFont(string $family, string $style='', string $file='') : void {
		...
		//--
		$ttfstat = \stat((string)\SmartFileSysUtils::getSmartFsRootPath().$ttffilename);
		if(!\is_array($ttfstat)) {
		...
		$uxmCacheFName = (string) self::ZTTFontCachePath.'fontAfm-'.\Smart::safe_varname((string)$b62Hash).'.json';
		if($this->uxmUseFontCache === true) {
			if(\SmartFileSysUtils::pathExists((string)self::ZTTFontCachePath) !== true) {
	...
	private function _parsejpg(string $file) : array {
		...
		//--
		if(!\is_file((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('JPG image file not found: '.$file);
			return [];
		} //end if
		if(!\is_readable((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('JPG image file not readable: '.$file);
			return [];
		} //end if
		//--
		$a = \getimagesize((string)\SmartFileSysUtils::getSmartFsRootPath().$file); // {{{SYNC-SF-STATIC-FILES-ROOT}}}
		if(!$a) {
		...
		//--
		return [ 'w'=>($a[0] ?? null), 'h'=>($a[1] ?? null), 'cs'=>$colspace, 'bpc'=>$bpc, 'f'=>'DCTDecode', 'data'=>$data ];
		//--
	} //END FUNCTION
	...
	private function _parsegif(string $file) : array {
		...
		//--
		if(!\is_file((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('GIF image file not found: '.$file);
			return [];
		} //end if
		if(!\is_readable((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('GIF image file not readable: '.$file);
			return [];
		} //end if
		//--
		$im = \imagecreatefromgif((string)\SmartFileSysUtils::getSmartFsRootPath().$file); // {{{SYNC-SF-STATIC-FILES-ROOT}}}
		if(!$im) {
			$this->Error('GIF image: Invalid file: '.$file);
			return [];
		} //end if
		//--
		\imageinterlace($im, 0);
	...
	private function _parsepng(string $file) : array {
		...
		//--
		if(!\is_file((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('PNG image file not found: '.$file);
			return [];
		} //end if
		if(!\is_readable((string)\SmartFileSysUtils::getSmartFsRootPath().$file)) {
			$this->Error('PNG image file not readable: '.$file);
			return [];
		} //end if
		//--
		$f = \fopen((string)\SmartFileSysUtils::getSmartFsRootPath().$file, 'rb'); // {{{SYNC-SF-STATIC-FILES-ROOT}}}
		if(!$f) {
			$this->Error('PNG image: Failed to open file: '.$file);
			return [];
		} //end if
		//--
		$info = (array) $this->_parsepngstream($f, (string)$file);
