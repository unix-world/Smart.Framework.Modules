<?php

// [===]+[!==] ready

namespace PDF\zFPDF;

//-- class:
// zTTFontFile (based on TTFontFile 1.06, from tFPDF 1.33)
// Version: 1.06.uxm
// Date:     2026-08-21
// Authors:  Radu Ovidiu I. <iradu@unix-world.org>
// Copyright (c) unix-world.org, 2026-present
// License:  aGPLv3 (GNU AFFERO GENERAL PUBLIC LICENSE Version 3)
//-- based on class:
// TTFontFile (PHP), based on The ReportLab Open Source PDF library (written in Python - http://www.reportlab.com/software/opensource/ - together with ideas from the OpenOffice source code and others)
// Version:  1.06
// Date:     2022-12-20
// Authors:  Ian Back <ianb@bpm1.com>
// Copyright (c) Ian Back, 2010
// License:  LGPL
//-- #


final class zTTFontFile {

	// PHP 8.1 or later
	// depends on classes: SmartFileSysUtils

	public const VERSION = '1.06.uxm.20260821';

	// Define the value used in the 'head' table of a created TTF file
	// 0x74727565 TRUE for Mac
	// 0x00010000 for Windows
	// Either seems to work for a font embedded in a PDF file when read by Adobe Reader on a Windows PC(!)
	private const TTF_MAC_HEADER = true; // unixman changed to TRUE from FALSE

	// TrueType Font Glyph operators
	private const GLYPH_WORDS 		= (1 << 0);
	private const GLYPH_SCALE 		= (1 << 3);
	private const GLYPH_MORE  		= (1 << 5);
	private const GLYPH_XYSCALE 	= (1 << 6);
	private const GLYPH_TWOBYTWO 	= (1 << 7);


	public $maxUni;
	public $maxUniChar;
	public $sFamilyClass;
	public $sFamilySubClass;
	public $_pos;
	public $numTables;
	public $searchRange;
	public $entrySelector;
	public $rangeShift;
	public $tables;
	public $otables;
	public $filename;
	public $fh;
	public $hmetrics;
	public $glyphPos;
	public $charToGlyph;
	public $codeToGlyph;
	public $glyphdata;
	public $ascent;
	public $descent;
	public $TTCFonts;
	public $version;
	public $name;
	public $familyName;
	public $styleName;
	public $fullName;
	public $uniqueFontID;
	public $unitsPerEm;
	public $bbox;
	public $capHeight;
	public $stemV;
	public $italicAngle;
	public $flags;
	public $underlinePosition;
	public $underlineThickness;
	public $charWidths;
	public $defaultWidth;
	public $maxStrLenRead;


	//-------- [PUBLIC METHODS]


	public function __construct() {
		//--
		$this->maxStrLenRead = 200000; // Maximum size of glyf table to read in as string (otherwise reads each glyph from file)
		//--
	} //END FUNCTION


	public function getMetrics(string $file) : void {
		//--
		$this->filename = (string) \trim((string)$file);
		if((string)$this->filename === '') {
			throw new \Exception('getMetrics: File Name is Empty');
			return;
		} //end if
		//--
		if(\str_ends_with((string)$this->filename, '.ttf') !== true) {
			throw new \Exception('getMetrics: Invalid TTF File Extension: `'.$this->filename.'`');
			return;
		} //end if
		//--
		if(\SmartFileSysUtils::checkIfSafePath((string)$file) !== 1) {
			throw new \Exception('getMetrics: Unsafe TTF File Path: `'.$this->filename.'`');
			return;
		} //end if
		//--
		if(!\is_file((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename)) {
			throw new \Exception('getMetrics: TTF file not found: `'.$this->filename.'`');
			return;
		} //end if
		if(!\is_readable((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename)) {
			throw new \Exception('getMetrics: TTF file not readable: `'.$this->filename.'`');
			return;
		} //end if
		//--
		$this->fh = \fopen((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename, 'rb'); // {{{SYNC-SF-STATIC-FILES-ROOT}}}
		if(!$this->fh) {
			throw new \Exception('getMetrics: Cannot Open File: `'.$this->filename.'`');
			return;
		} //end if
		//--
		$this->_pos 		= 0;
		$this->charWidths 	= '';
		$this->glyphPos 	= [];
		$this->charToGlyph 	= [];
		$this->tables 		= [];
		$this->otables 		= [];
		$this->ascent 		= 0;
		$this->descent 		= 0;
		$this->TTCFonts 	= [];
		$this->version 		= (int) $this->read_ulong();
		//--
		if((int)$this->version === (int)\intval(0x4F54544F)) {
			throw new \Exception('getMetrics: Postscript outlines are not supported');
			return;
		} //end if
		if((int)$this->version === (int)\intval(0x74746366)) {
			throw new \Exception('getMetrics: TrueType Fonts Collections not supported');
			return;
		} //end if
		if(!in_array((int)$this->version, [ \intval(0x00010000), \intval(0x74727565) ])) {
			throw new \Exception('getMetrics: Not a supported TrueType font: version='.$this->version);
			return;
		} //end if
		//--
		$this->readTableDirectory();
		$this->extractInfo();
		//--
		\fclose($this->fh);
		//--
	} //END FUNCTION


	public function makeSubset(string $file, array $subset) : string { // unixman
		//--
		$this->filename = (string) \trim((string)$file);
		if((string)$this->filename === '') {
			throw new \Exception('makeSubset: File Name is Empty');
			return '';
		} //end if
		//--
		if(\str_ends_with((string)$this->filename, '.ttf') !== true) {
			throw new \Exception('makeSubset: Invalid TTF File Extension: `'.$this->filename.'`');
			return '';
		} //end if
		//--
		if(\SmartFileSysUtils::checkIfSafePath((string)$file) !== 1) {
			throw new \Exception('makeSubset: Unsafe TTF File Path: `'.$this->filename.'`');
			return '';
		} //end if
		//--
		if(!\is_file((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename)) {
			throw new \Exception('makeSubset: TTF file not found: `'.$this->filename.'`');
			return '';
		} //end if
		if(!\is_readable((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename)) {
			throw new \Exception('makeSubset: TTF file not readable: `'.$this->filename.'`');
			return '';
		} //end if
		//--
		$this->fh = \fopen((string)\SmartFileSysUtils::getSmartFsRootPath().$this->filename, 'rb'); // {{{SYNC-SF-STATIC-FILES-ROOT}}}
		if(!$this->fh) {
			throw new \Exception('makeSubset: Cannot Open File: `'.$this->filename.'`');
			return '';
		} //end if
		//--
		$this->_pos = 0;
		$this->charWidths = '';
		$this->glyphPos = [];
		$this->charToGlyph = [];
		$this->tables = [];
		$this->otables = [];
		$this->ascent = 0;
		$this->descent = 0;
		$this->skip(4);
		$this->maxUni = 0;
		$this->readTableDirectory();
		//-- head - Font header table
		$this->seek_table('head');
		$this->skip(50);
		$indexToLocFormat = $this->read_ushort();
		$glyphDataFormat = $this->read_ushort();
		//-- hhea - Horizontal header table
		$this->seek_table('hhea');
		$this->skip(32);
		$metricDataFormat = $this->read_ushort();
		$orignHmetrics = $numberOfHMetrics = $this->read_ushort();
		//-- maxp - Maximum profile table
		$this->seek_table('maxp');
		$this->skip(4);
		$numGlyphs = $this->read_ushort();
		//-- cmap - Character to glyph index mapping table
		$cmap_offset = $this->seek_table('cmap');
		$this->skip(2);
		$cmapTableCount = $this->read_ushort();
		$unicode_cmap_offset = 0;
		//--
		for($i=0; $i<$cmapTableCount; $i++) {
			$platformID = $this->read_ushort();
			$encodingID = $this->read_ushort();
			$offset = $this->read_ulong();
			$save_pos = $this->_pos;
			if((((int)$platformID === 3) && ((int)$encodingID === 1)) || ((int)$platformID === 0)) { // Microsoft, Unicode
				$format = $this->get_ushort($cmap_offset + $offset);
				if((int)$format === 4) {
					$unicode_cmap_offset = $cmap_offset + $offset;
					break;
				} //end if
			} //end if
			$this->seek($save_pos);
		} //end for
		//--
		if(!$unicode_cmap_offset) {
			throw new \Exception('makeSubset: Font ('.$this->filename .') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)');
			return '';
		} //end if
		//--
		$glyphToChar = [];
		$charToGlyph = [];
		$this->getCMAP4($unicode_cmap_offset, $glyphToChar, $charToGlyph);
		//--
		$this->charToGlyph = $charToGlyph;
		//-- hmtx - Horizontal metrics table
		$scale = 1;	// not used
		$this->getHMTX($numberOfHMetrics, $numGlyphs, $glyphToChar, $scale);
		//-- loca - Index to location
		$this->getLOCA($indexToLocFormat, $numGlyphs);
		//--
		$subsetglyphs = [ 0 => 0 ];
		$subsetCharToGlyph = [];
		//--
		foreach($subset as $code) {
			if(isset($this->charToGlyph[$code])) {
				$subsetglyphs[$this->charToGlyph[$code]] = $code;	// Old Glyph ID => Unicode
				$subsetCharToGlyph[$code] = $this->charToGlyph[$code];	// Unicode to old GlyphID
			} //end if
			$this->maxUni = \max($this->maxUni, $code);
		} //end foreach
		//--
		list($start, $dummy) = $this->get_table_pos('glyf');
		//--
		$glyphSet = array();
		\ksort($subsetglyphs);
		$n = 0;
		$fsLastCharIndex = 0;	// maximum Unicode index (character code) in this font, according to the cmap subtable for platform ID 3 and platform- specific encoding ID 0 or 1.
		foreach($subsetglyphs as $originalGlyphIdx => $uni) {
			$fsLastCharIndex = \max($fsLastCharIndex , $uni);
			$glyphSet[$originalGlyphIdx] = $n;	// old glyphID to new glyphID
			$n++;
		} //end foreach
		//--
		\ksort($subsetCharToGlyph);
		$codeToGlyph = []; // unixman: bug fix, it was missing
		foreach($subsetCharToGlyph as $uni => $originalGlyphIdx) {
			$codeToGlyph[$uni] = $glyphSet[$originalGlyphIdx] ;
		} //end foreach
		$this->codeToGlyph = $codeToGlyph;
		//--
		\ksort($subsetglyphs);
		foreach($subsetglyphs as $originalGlyphIdx => $uni) {
			$this->getGlyphs($originalGlyphIdx, $start, $glyphSet, $subsetglyphs);
		} //end foreach
		//--
		$numGlyphs = $numberOfHMetrics = \count($subsetglyphs);
		//-- tables copied from the original
		$tags = [ 'name' ];
		foreach($tags as $tag) {
			$this->add($tag, $this->get_table($tag));
		} //end foreach
		$tags = [ 'cvt ', 'fpgm', 'prep', 'gasp' ];
		foreach($tags as $tag) {
			if(isset($this->tables[$tag])) {
				$this->add($tag, $this->get_table($tag));
			} //end if
		} //end foreach
		//-- post - PostScript
		$opost = $this->get_table('post');
		$post = "\x00\x03\x00\x00".\substr((string)$opost, 4, 12)."\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
		$this->add('post', $post);
		//-- Sort CID2GID map into segments of contiguous codes
		\ksort($codeToGlyph);
		unset($codeToGlyph[0]);
		//unset($codeToGlyph[65535]);
		$rangeid = 0;
		$range = array();
		$prevcid = -2;
		$prevglidx = -1;
		//-- for each character
		foreach($codeToGlyph as $cid => $glidx) {
			if(((int)$cid === (int)((int)$prevcid + 1)) && ((int)$glidx === (int)($prevglidx + 1))) {
				$range[$rangeid][] = $glidx;
			} else { // new range
				$rangeid = $cid;
				$range[$rangeid] = [];
				$range[$rangeid][] = $glidx;
			} //end if else
			$prevcid = $cid;
			$prevglidx = $glidx;
		} //end foreach
		//-- cmap - Character to glyph mapping - Format 4 (MS)
		$segCount = \count($range) + 1;	// + 1 Last segment has missing character 0xFFFF
		$searchRange = 1;
		$entrySelector = 0;
		while($searchRange * 2 <= $segCount) {
			$searchRange = $searchRange * 2;
			$entrySelector = $entrySelector + 1;
		} //end while
		$searchRange = $searchRange * 2;
		$rangeShift = $segCount * 2 - $searchRange;
		$length = 16 + (8 * $segCount) + ($numGlyphs + 1);
		$cmap = [
			0, 1,			// Index : version, number of encoding subtables
			3, 1,			// Encoding Subtable : platform (MS=3), encoding (Unicode)
			0, 12,			// Encoding Subtable : offset (hi,lo)
			4, $length, 0, 	// Format 4 Mapping subtable: format, length, language
			$segCount*2,
			$searchRange,
			$entrySelector,
			$rangeShift
		];
		//-- endCode(s)
		foreach($range as $start=>$subrange) {
			$endCode = $start + (\count($subrange)-1);
			$cmap[] = $endCode;	// endCode(s)
		} //end foreach
		$cmap[] = 0xFFFF; // endCode of last Segment
		$cmap[] = 0; 	  // reservedPad
		//-- startCode(s)
		foreach($range as $start=>$subrange) {
			$cmap[] = $start; // startCode(s)
		} //end foreach
		$cmap[] = 0xFFFF; // startCode of last Segment
		// idDelta(s)
		foreach($range as $start => $subrange) {
			$idDelta = -1 * ($start - $subrange[0]);
			$n += \count($subrange);
			$cmap[] = $idDelta;	// idDelta(s)
		} //end foreach
		$cmap[] = 1; // idDelta of last Segment
		//-- idRangeOffset(s)
		foreach($range as $subrange) {
			$cmap[] = 0; // idRangeOffset[segCount] Offset in bytes to glyph indexArray, or 0
		} //end foreach
		$cmap[] = 0; // idRangeOffset of last Segment
		foreach($range as $subrange) {
			foreach($subrange as $glidx) {
				$cmap[] = $glidx;
			} //end foreach
		} //end foreach
		$cmap[] = 0; // Mapping for last character
		$cmapstr = '';
		foreach($cmap as $cm) {
			$cmapstr .= \pack('n', $cm);
		} //end foreach
		$this->add('cmap', $cmapstr);
		//-- glyf - Glyph data
		list($glyfOffset, $glyfLength) = $this->get_table_pos('glyf');
		if($glyfLength < $this->maxStrLenRead) {
			$glyphData = $this->get_table('glyf');
		} //end if
		//--
		$offsets = [];
		$glyf = '';
		$pos = 0;
		//--
		$hmtxstr = '';
		$xMinT = 0;
		$yMinT = 0;
		$xMaxT = 0;
		$yMaxT = 0;
		$advanceWidthMax = 0;
		$minLeftSideBearing = 0;
		$minRightSideBearing = 0;
		$xMaxExtent = 0;
		$maxPoints = 0;				// points in non-compound glyph
		$maxContours = 0;			// contours in non-compound glyph
		$maxComponentPoints = 0;	// points in compound glyph
		$maxComponentContours = 0;	// contours in compound glyph
		$maxComponentElements = 0;	// number of glyphs referenced at top level
		$maxComponentDepth = 0;		// levels of recursion, set to 0 if font has only simple glyphs
		$this->glyphdata = [];
		//--
		foreach($subsetglyphs as $originalGlyphIdx => $uni) {
			//-- hmtx - Horizontal Metrics
			$hm = $this->getHMetric($orignHmetrics, $originalGlyphIdx);
			$hmtxstr .= $hm;
			//--
			$offsets[] = $pos;
			$glyphPos = $this->glyphPos[$originalGlyphIdx];
			$glyphLen = $this->glyphPos[$originalGlyphIdx + 1] - $glyphPos;
			if($glyfLength < $this->maxStrLenRead) {
				$data = \substr($glyphData, $glyphPos, $glyphLen);
			} else {
				if($glyphLen > 0) {
					$data = $this->get_chunk($glyfOffset + $glyphPos, $glyphLen);
				} else {
					$data = '';
				} //end if else
			} //end if else
			//--
			if($glyphLen > 0) {
				$up = \unpack('n', \substr((string)$data, 0, 2));
			} //end if
			//--
			if($glyphLen > 2 && ($up[1] & (1 << 15))) {	// If number of contours <= -1 i.e. composite glyph
				$pos_in_glyph = 10;
				$flags = self::GLYPH_MORE;
				$nComponentElements = 0;
				while($flags & self::GLYPH_MORE) {
					$nComponentElements += 1;	// number of glyphs referenced at top level
					$up = \unpack('n', \substr($data, $pos_in_glyph, 2));
					$flags = $up[1];
					$up = \unpack('n', \substr($data, $pos_in_glyph + 2, 2));
					$glyphIdx = $up[1];
					$this->glyphdata[$originalGlyphIdx]['compGlyphs'][] = $glyphIdx;
					$data = $this->_set_ushort($data, $pos_in_glyph + 2, $glyphSet[$glyphIdx]);
					$pos_in_glyph += 4;
					if($flags & self::GLYPH_WORDS) {
						$pos_in_glyph += 4;
					} else {
						$pos_in_glyph += 2;
					} //end if else
					if($flags & self::GLYPH_SCALE) {
						$pos_in_glyph += 2;
					} elseif($flags & self::GLYPH_XYSCALE) {
						$pos_in_glyph += 4;
					} elseif($flags & self::GLYPH_TWOBYTWO) {
						$pos_in_glyph += 8;
					} //end if else
				} //end while
				$maxComponentElements = max($maxComponentElements, $nComponentElements);
			} //end if
			//--
			$glyf .= (string) $data;
			$pos += (int) $glyphLen;
			if((int)((int)$pos % 4) !== 0) {
				$padding = (int) (4 - (int)((int)$pos % 4));
				$glyf .= (string) \str_repeat("\0", $padding);
				$pos += (int) $padding;
			} //end if
		} //end foreach
		//--
		$offsets[] = (int) $pos;
		$this->add('glyf', (string)$glyf);
		//-- hmtx - Horizontal Metrics
		$this->add('hmtx', (string)$hmtxstr);
		//-- loca - Index to location
		$locastr = '';
		if((($pos + 1) >> 1) > 0xFFFF) {
			$indexToLocFormat = 1; // long format
			foreach($offsets as $offset) {
				$locastr .= \pack('N', $offset);
			} //end foreach
		} else {
			$indexToLocFormat = 0; // short format
			foreach($offsets as $offset) {
				$locastr .= \pack('n', ($offset / 2));
			} //end foreach
		} //end if else
		$this->add('loca', (string)$locastr);
		//-- head - Font header
		$head = $this->get_table('head');
		$head = $this->_set_ushort($head, 50, $indexToLocFormat);
		$this->add('head', (string)$head);
		//-- hhea - Horizontal Header
		$hhea = $this->get_table('hhea');
		$hhea = $this->_set_ushort($hhea, 34, $numberOfHMetrics);
		$this->add('hhea', (string)$hhea);
		//-- maxp - Maximum Profile
		$maxp = $this->get_table('maxp');
		$maxp = $this->_set_ushort($maxp, 4, $numGlyphs);
		$this->add('maxp', (string)$maxp);
		//-- OS/2 - OS/2
		$os2 = $this->get_table('OS/2');
		$this->add('OS/2', (string)$os2);
		//--
		\fclose($this->fh);
		//-- Put the TTF file together
	//	$stm = '';
	//	$this->endTTFile($stm);
	//	return (string) $stm ;
		return (string) $this->endTTFile(); // unixman
		//--
	} //END FUNCTION


	//-------- [ PRIVATE METHODS ]


	private function readTableDirectory() : void {
		//--
		$this->numTables = $this->read_ushort();
		$this->searchRange = $this->read_ushort();
		$this->entrySelector = $this->read_ushort();
		$this->rangeShift = $this->read_ushort();
		$this->tables = [];
		//--
		for($i=0; $i<$this->numTables; $i++) {
			$record = [];
			$record['tag'] = $this->read_tag();
			if((string)$record['tag'] === '') {
				throw new \Exception('Font: Failed to read Table directory'); // unixman
				return;
			} //end if
			$record['checksum'] = array($this->read_ushort(),$this->read_ushort());
			$record['offset'] = $this->read_ulong();
			$record['length'] = $this->read_ulong();
			$this->tables[$record['tag']] = $record;
		} //end for
		//--
	} //END FUNCTION


	private function sub32(array $x, array $y) : array {
		//--
		$xlo = $x[1];
		$xhi = $x[0];
		$ylo = $y[1];
		$yhi = $y[0];
		//--
		if($ylo > $xlo) {
			$xlo += 1 << 16; $yhi += 1;
		} //end if
		//--
		$reslo = $xlo - $ylo;
		//--
		if($yhi > $xhi) {
			$xhi += 1 << 16;
		} //end if
		//--
		$reshi = $xhi - $yhi;
		$reshi = $reshi & 0xFFFF;
		//--
		return [ $reshi, $reslo ];
		//--
	} //END FUNCTION


	private function calcChecksum(?string $data) : array  {
		//--
		$data = (string) $data;
		//--
		if(\strlen($data) % 4) {
			$data .= \str_repeat("\0", (4 - (\strlen($data) % 4)));
		} //end if
		//--
		$hi=0x0000;
		$lo=0x0000;
		for($i=0;$i<\strlen($data);$i+=4) {
			$hi += (\ord($data[$i]) << 8)   + \ord($data[$i+1]);
			$lo += (\ord($data[$i+2]) << 8) + \ord($data[$i+3]);
			$hi += $lo >> 16;
			$lo = $lo & 0xFFFF;
			$hi = $hi & 0xFFFF;
		} //end for
		//--
		return [ $hi, $lo ];
		//--
	} //END FUNCTION


	private function get_table_pos(string $tag) {
		//--
		$offset = $this->tables[$tag]['offset'];
		$length = $this->tables[$tag]['length'];
		//--
		return [ $offset, $length ];
		//--
	} //END FUNCTION


	private function seek(int $pos) : void {
		//--
		$this->_pos = $pos;
		\fseek($this->fh, $this->_pos);
		//--
	} //END FUNCTION


	private function skip(int $delta) : void {
		//--
		$this->_pos = $this->_pos + $delta;
		\fseek($this->fh, $this->_pos);
		//--
	} //END FUNCTION


	private function seek_table(string $tag, int $offset_in_table=0) : int {
		//--
		$tpos = $this->get_table_pos($tag);
		$this->_pos = $tpos[0] + $offset_in_table;
		\fseek($this->fh, $this->_pos);
		//--
		return (int) $this->_pos;
		//--
	} //END FUNCTION


	private function read_tag() : string {
		//--
		$this->_pos += 4;
		//--
		return (string) \fread($this->fh,4);
		//--
	} //END FUNCTION


	private function read_short() : int {
		//--
		$this->_pos += 2;
		$s = \fread($this->fh,2);
		$a = (\ord($s[0]) << 8) + \ord($s[1]);
		if($a & (1 << 15)) {
			$a = ($a - (1 << 16));
		} //end if
		//--
		return (int) $a;
		//--
	} //END FUNCTION


	/* unixman: unused
	private function unpack_short(string $s) : int {
		//--
		$a = (\ord($s[0]) << 8) + \ord($s[1]);
		if($a & (1 << 15)) {
			$a = ($a - (1 << 16));
		} //end if
		//--
		return (int) $a;
		//--
	} //END FUNCTION


	private function pack_short(int $val) : string {
		//--
		if($val < 0) {
			$val = \abs($val);
			$val = ~$val;
			$val += 1;
		} //end if
		//--
		return (string) \pack('n', $val);
		//--
	} //END FUNCTION
	*/


	private function read_ushort() : int {
		//--
		$this->_pos += 2;
		$s = \fread($this->fh,2);
		//--
		return (int) (\ord($s[0]) << 8) + \ord($s[1]);
		//--
	} //END FUNCTION


	private function get_ushort(int $pos) : int {
		//--
		\fseek($this->fh, $pos);
		$s = \fread($this->fh,2);
		//--
		return (int) (\ord($s[0]) << 8) + \ord($s[1]);
		//--
	} //END FUNCTION


	private function read_ulong() : int {
		//--
		$this->_pos += 4;
		$s = \fread($this->fh,4); // if large uInt32 as an integer, PHP converts it to -ve
		//--
		return (int) (\ord($s[0]) * 16777216) + (\ord($s[1]) << 16) + (\ord($s[2]) << 8) + \ord($s[3]); // 16777216 = 1 << 24
		//--
	} //END FUNCTION


	/* unixman: unused
	private function get_ulong(int $pos) : int {
		//--
		\fseek($this->fh, $pos);
		$s = \fread($this->fh, 4); // iF large uInt32 as an integer, PHP converts it to -ve
		//--
		return (int) (\ord($s[0]) * 16777216) + (\ord($s[1]) << 16) + (\ord($s[2]) << 8) + \ord($s[3]); // 16777216 = 1 << 24
		//--
	} //END FUNCTION
	*/


	private function splice(?string $stream, int $offset, ?string $value) : string {
		//--
		return (string) \substr((string)$stream, 0, (int)$offset) . (string)$value . \substr((string)$stream, (int)$offset + (int)\strlen((string)$value));
		//--
	} //END FUNCTION


	private function _set_ushort(?string $stream, int $offset, ?string $value) : string {
		//--
		$up = (string) \pack('n', $value);
		//--
		return (string) $this->splice((string)$stream, (int)$offset, (string)$up);
		//--
	} //END FUNCTION


	/* unixman: unused
	private function _set_short(?string $stream, int $offset, int $val) : string {
		//--
		if($val < 0) {
			$val = \abs($val);
			$val = ~$val;
			$val += 1;
		} //end if
		//--
		$up = (string) \pack('n', $val);
		//--
		return (string) $this->splice((string)$stream, (int)$offset, (string)$up);
		//--
	} //END FUNCTION
	*/


	private function get_chunk(int $pos, int $length) : string {
		//--
		\fseek($this->fh, $pos);
		if($length < 1) {
			return '';
		} //end if
		//--
		$out = \fread($this->fh,$length);
		if($out === false) {
			throw new \Exception('get_chunk: Invalid Read: p='.$pos.' / l='.$length);
			return '';
		} //end if
		//--
		return (string) $out;
		//--
	} //END FUNCTION


	private function get_table(string $tag) : string {
		//--
		list($pos, $length) = $this->get_table_pos($tag);
		//--
		if($length <= 0) {
			throw new \Exception('Truetype font ('.$this->filename.'): error reading table: '.$tag);
			return '';
		} //end if
		//--
		\fseek($this->fh, $pos);
		//--
		return (string) \fread($this->fh, $length);
		//--
	} //END FUNCTION


	private function add(string $tag, string $data) : void {
		//--
		if($tag === 'head') {
			$data = $this->splice($data, 8, "\0\0\0\0");
		} //end if
		//--
		$this->otables[$tag] = $data;
		//--
	} //END FUNCTION


	private function extractInfo() : void {
		//--
		// name - Naming table
		//--
		$this->sFamilyClass = 0;
		$this->sFamilySubClass = 0;
		//--
		$name_offset = $this->seek_table('name');
		$format = $this->read_ushort();
		if((int)$format !== 0) {
			throw new \Exception('Unknown name table format: '.$format);
			return;
		} //end if
		//--
		$numRecords = $this->read_ushort();
		$string_data_offset = $name_offset + $this->read_ushort();
		$names = [ 1=>'', 2=>'', 3=>'', 4=>'', 6=>'' ];
		$K = \array_keys($names);
		$nameCount = \count($names);
		for($i=0; $i<$numRecords; $i++) {
			//--
			$platformId = $this->read_ushort();
			$encodingId = $this->read_ushort();
			$languageId = $this->read_ushort();
			$nameId = $this->read_ushort();
			$length = $this->read_ushort();
			$offset = $this->read_ushort();
			//--
			if(!\in_array($nameId,$K)) {
				continue;
			} //end if
			//--
			$N = '';
			if(((int)$platformId === 3) && ((int)$encodingId === 1) && ((int)$languageId === (int)\intval(0x409))) { // Microsoft, Unicode, US English, PS Name
				//--
				$opos = $this->_pos;
				$this->seek($string_data_offset + $offset);
				//--
				if((int)((int)$length % 2) !== 0) {
					throw new \Exception('PostScript name is UTF-16BE string of odd length');
					return;
				} //end if
				//--
				$length /= 2;
				$N = '';
				//--
				while($length > 0) {
					$char = $this->read_ushort();
					$N .= \chr($char);
					$length -= 1;
				} //end while
				//--
				$this->_pos = $opos;
				$this->seek($opos);
				//--
			} elseif(((int)$platformId === 1) && ((int)$encodingId === 0) && ((int)$languageId === 0)) { // Macintosh, Roman, English, PS Name
				//--
				$opos = $this->_pos;
				$N = $this->get_chunk($string_data_offset + $offset, $length);
				$this->_pos = $opos;
				$this->seek($opos);
				//--
			} //end if else
			//--
			if($N && ((string)$names[$nameId] === '')) {
				//--
				$names[$nameId] = $N;
				$nameCount -= 1;
				if((int)$nameCount === 0) {
					break;
				} //end if
				//--
			} //end if
			//--
		} //end for
		//--
		if($names[6]) {
			$psName = $names[6];
		} elseif($names[4]) {
			$psName = \preg_replace('/ /', '-', $names[4]);
		} elseif($names[1]) {
			$psName = \preg_replace('/ /', '-', $names[1]);
		} else {
			$psName = '';
		} //end if else
		//--
		if(!$psName) {
			throw new \Exception('Could not find PostScript font name');
			return;
		} //end if
		//--
		$this->name = $psName;
		//--
		if($names[1]) {
			$this->familyName = $names[1];
		} else {
			$this->familyName = $psName;
		} //end if else
		if($names[2]) {
			$this->styleName = $names[2];
		} else {
			$this->styleName = 'Regular';
		} //end if else
		if($names[4]) {
			$this->fullName = $names[4];
		} else {
			$this->fullName = $psName;
		} //end if else
		if($names[3]) {
			$this->uniqueFontID = $names[3];
		} else {
			$this->uniqueFontID = $psName;
		} //end if else
		if($names[6]) {
			$this->fullName = $names[6];
		} //end if
		//-- head - Font header table
		$this->seek_table('head');
		$this->skip(18);
		$this->unitsPerEm = $unitsPerEm = $this->read_ushort();
		$scale = 1000 / $unitsPerEm;
		$this->skip(16);
		$xMin = $this->read_short();
		$yMin = $this->read_short();
		$xMax = $this->read_short();
		$yMax = $this->read_short();
		$this->bbox = [ ($xMin * $scale), ($yMin * $scale), ($xMax * $scale), ($yMax * $scale) ];
		$this->skip(3 * 2);
		$indexToLocFormat = $this->read_ushort();
		$glyphDataFormat = $this->read_ushort();
		if((int)$glyphDataFormat !== 0) {
			throw new \Exception('Unknown glyph data format: '.$glyphDataFormat);
			return;
		} //end if
		// hhea metrics table ; ttf2t1 seems to use this value rather than the one in OS/2 - so put in for compatibility
		if(isset($this->tables['hhea'])) {
			$this->seek_table('hhea');
			$this->skip(4);
			$hheaAscender = $this->read_short();
			$hheaDescender = $this->read_short();
			$this->ascent = ($hheaAscender * $scale);
			$this->descent = ($hheaDescender * $scale);
		} //end if
		//-- OS/2 - OS/2 and Windows metrics table
		if(isset($this->tables['OS/2'])) {
			$this->seek_table('OS/2');
			$version = $this->read_ushort();
			$this->skip(2);
			$usWeightClass = $this->read_ushort();
			$this->skip(2);
			$fsType = $this->read_ushort();
			if(
				((int)$fsType === (int)\intval(0x0002))
				||
				((int)\intval($fsType & 0x0300) !== 0)
			) {
				throw new \Exception('FAILED, Font file '.$this->filename.' cannot be embedded due to copyright restrictions.');
				return;
			} //end if
			$this->skip(20);
			$sF = $this->read_short();
			$this->sFamilyClass = ($sF >> 8);
			$this->sFamilySubClass = ($sF & 0xFF);
			$this->_pos += 10;  //PANOSE = 10 byte length
			$panose = \fread($this->fh,10);
			$this->skip(26);
			$sTypoAscender = $this->read_short();
			$sTypoDescender = $this->read_short();
			if(!$this->ascent) {
				$this->ascent = ($sTypoAscender * $scale);
			} //end if
			if(!$this->descent) {
				$this->descent = ($sTypoDescender * $scale);
			} //end if
			if($version > 1) {
				$this->skip(16);
				$sCapHeight = $this->read_short();
				$this->capHeight = ($sCapHeight * $scale);
			} else {
				$this->capHeight = $this->ascent;
			} //end if else
		} else {
			$usWeightClass = 500;
			if(!$this->ascent) {
				$this->ascent = ($yMax * $scale);
			} //end if
			if(!$this->descent) {
				$this->descent = ($yMin * $scale);
			} //end if
			$this->capHeight = $this->ascent;
		} //end if else
		$this->stemV = 50 + \intval(\pow(($usWeightClass / 65.0), 2));
		//-- post - PostScript table
		$this->seek_table('post');
		$this->skip(4);
		$this->italicAngle = $this->read_short() + $this->read_ushort() / 65536.0;
		$this->underlinePosition = $this->read_short() * $scale;
		$this->underlineThickness = $this->read_short() * $scale;
		$isFixedPitch = $this->read_ulong();
		//--
		$this->flags = 4;
		//--
		if($this->italicAngle!= 0) {
			$this->flags = $this->flags | 64;
		} //end if
		if($usWeightClass >= 600) {
			$this->flags = $this->flags | 262144;
		} //end if
		if($isFixedPitch) {
			$this->flags = $this->flags | 1;
		} //end if
		//-- hhea - Horizontal header table
		$this->seek_table('hhea');
		$this->skip(32);
		$metricDataFormat = $this->read_ushort();
		if((int)$metricDataFormat !== 0) {
			throw new \Exception('Unknown horizontal metric data format '.$metricDataFormat);
			return;
		} //end if
		$numberOfHMetrics = $this->read_ushort();
		if((int)$numberOfHMetrics === 0) {
			throw new \Exception('Number of horizontal metrics is 0');
			return;
		} //end if
		//-- maxp - Maximum profile table
		$this->seek_table('maxp');
		$this->skip(4);
		$numGlyphs = $this->read_ushort();
		//-- cmap - Character to glyph index mapping table
		$cmap_offset = $this->seek_table('cmap');
		$this->skip(2);
		$cmapTableCount = $this->read_ushort();
		$unicode_cmap_offset = 0;
		for($i=0; $i<$cmapTableCount; $i++) {
			$platformID = $this->read_ushort();
			$encodingID = $this->read_ushort();
			$offset = $this->read_ulong();
			$save_pos = $this->_pos;
			if((((int)$platformID === 3) && ((int)$encodingID === 1)) || ((int)$platformID === 0)) { // Microsoft, Unicode
				$format = $this->get_ushort($cmap_offset + $offset);
				if((int)$format === 4) {
					if(!$unicode_cmap_offset) {
						$unicode_cmap_offset = $cmap_offset + $offset;
					} //end if
					break;
				} //end if
			} //end if
			$this->seek($save_pos);
		} //end for
		if(!$unicode_cmap_offset) {
			throw new \Exception('FAILED, Font ('.$this->filename .') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)');
		} //end if
		//--
		$glyphToChar = [];
		$charToGlyph = [];
		$this->getCMAP4($unicode_cmap_offset, $glyphToChar, $charToGlyph);
		//-- hmtx - Horizontal metrics table
		$this->getHMTX($numberOfHMetrics, $numGlyphs, $glyphToChar, $scale);
		//--
	} //END FUNCTION


	/*
	private function getGlyphData(int $originalGlyphIdx, int &$maxdepth, int &$depth, int &$points, int &$contours) : void {
		//--
		// Recursively get composite glyph data
		//--
		$depth++;
		//--
		$maxdepth = \max($maxdepth, $depth);
		if(\count($this->glyphdata[$originalGlyphIdx]['compGlyphs'])) {
			foreach($this->glyphdata[$originalGlyphIdx]['compGlyphs'] as $glyphIdx) {
				$this->getGlyphData($glyphIdx, $maxdepth, $depth, $points, $contours);
			} //end foreach
		} elseif(($this->glyphdata[$originalGlyphIdx]['nContours'] > 0) && $depth > 0) { // simple
			$contours += $this->glyphdata[$originalGlyphIdx]['nContours'];
			$points += $this->glyphdata[$originalGlyphIdx]['nPoints'];
		} //end if else
		//--
		$depth--;
		//--
	} //END FUNCTION
	*/


	private function getGlyphs(int $originalGlyphIdx, int &$start, array &$glyphSet, array &$subsetglyphs) : void {
		//--
		// Recursively get composite glyphs
		//--
		$glyphPos = $this->glyphPos[$originalGlyphIdx];
		$glyphLen = $this->glyphPos[$originalGlyphIdx + 1] - $glyphPos;
		//--
		if(!$glyphLen) {
			return;
		} //end if
		//--
		$this->seek($start + $glyphPos);
		$numberOfContours = $this->read_short();
		if($numberOfContours < 0) {
			$this->skip(8);
			$flags = self::GLYPH_MORE;
			while($flags & self::GLYPH_MORE) {
				$flags = $this->read_ushort();
				$glyphIdx = $this->read_ushort();
				if(!isset($glyphSet[$glyphIdx])) {
					$glyphSet[$glyphIdx] = \count($subsetglyphs);	// old glyphID to new glyphID
					$subsetglyphs[$glyphIdx] = true;
				} //end if
				$savepos = \ftell($this->fh);
				$this->getGlyphs($glyphIdx, $start, $glyphSet, $subsetglyphs);
				$this->seek($savepos);
				if($flags & self::GLYPH_WORDS) {
					$this->skip(4);
				} else {
					$this->skip(2);
				} //end if else
				if($flags & self::GLYPH_SCALE) {
					$this->skip(2);
				} elseif($flags & self::GLYPH_XYSCALE) {
					$this->skip(4);
				} elseif($flags & self::GLYPH_TWOBYTWO) {
					$this->skip(8);
				} //end if else
			} //end while
		} //end if
		//--
	} // END FUNCTION


	private function getHMTX(int $numberOfHMetrics, int $numGlyphs, array &$glyphToChar, float $scale) : void {
		//--
		$start = $this->seek_table('hmtx');
		$aw = 0;
		$this->charWidths = (string) \str_pad('', 256*256*2, "\x00");
		$nCharWidths = 0;
		if(($numberOfHMetrics*4) < $this->maxStrLenRead) {
			$data = $this->get_chunk($start,($numberOfHMetrics*4));
			$arr = \unpack('n*', $data);
		} else {
			$this->seek($start);
		} //end if else
		//--
		for($glyph=0; $glyph<$numberOfHMetrics; $glyph++) {
			//--
			if(($numberOfHMetrics*4) < $this->maxStrLenRead) {
				$aw = $arr[($glyph*2)+1];
			} else {
				$aw = $this->read_ushort();
				$lsb = $this->read_ushort();
			} //end if else
			//--
			if(isset($glyphToChar[$glyph]) || ((int)$glyph === 0)) {
				if($aw >= (1 << 15)) {
					$aw = 0; // 1.03 Some (arabic) fonts have -ve values for width
				} //end if
				if((int)$glyph === 0) { // although should be unsigned value - comes out as e.g. 65108 (intended -50)
					$this->defaultWidth = $scale * $aw;
					continue;
				} //end if
				foreach($glyphToChar[$glyph] as $char) {
					if(((int)$char !== 0) && ((int)$char !== 65535)) {
 						$w = (int) \intval(\round($scale * $aw));
						if((int)$w === 0) {
							$w = 65535;
						} //end if
						if((int)$char < 196608) {
							$this->charWidths[$char*2]     = \chr($w >> 8);
							$this->charWidths[$char*2 + 1] = \chr($w & 0xFF);
							$nCharWidths++;
						} //end if
					} //end if
				} //end foreach
			} //end if
		} //end for
		//--
		$data = $this->get_chunk(($start + $numberOfHMetrics * 4), ($numGlyphs * 2));
		$arr = \unpack('n*', $data);
		$diff = $numGlyphs - $numberOfHMetrics;
		for($pos=0; $pos<$diff; $pos++) {
			$glyph = $pos + $numberOfHMetrics;
			if(isset($glyphToChar[$glyph])) {
				foreach($glyphToChar[$glyph] as $char) {
					if(((int)$char !== 0) && ((int)$char !== 65535)) {
						$w = (int) \intval(\round($scale * $aw));
						if((int)$w === 0) {
							$w = 65535;
						} //end if
						if((int)$char < 196608) {
							$this->charWidths[$char*2]     = \chr($w >> 8);
							$this->charWidths[$char*2 + 1] = \chr($w & 0xFF);
							$nCharWidths++;
						} //end if
					} //end if
				} //end foreach
			} //end if
		} //end for
		//-- NB 65535 is a set width of 0 First bytes define number of chars in font
		$this->charWidths[0] = \chr($nCharWidths >> 8);
		$this->charWidths[1] = \chr($nCharWidths & 0xFF);
		//--
	} // END FUNCTION


	private function getHMetric(int $numberOfHMetrics, int $gid) : string {
		//--
		$start = $this->seek_table('hmtx');
		//--
		$hm = '';
		if($gid < $numberOfHMetrics) {
			$this->seek($start + ($gid * 4));
			$hm = \fread($this->fh, 4);
		} else {
			$this->seek($start + (($numberOfHMetrics - 1) * 4));
			$hm = \fread($this->fh, 2);
			$this->seek($start + ($numberOfHMetrics * 2) + ($gid * 2));
			$hm .= \fread($this->fh, 2);
		} //end if else
		//--
		return (string) $hm;
		//--
	} //END FUNCTION


	private function getLOCA(int $indexToLocFormat, int $numGlyphs) : void {
		//--
		$start = $this->seek_table('loca');
		$this->glyphPos = [];
		//--
		if((int)$indexToLocFormat === 0) {
			//--
			$data = $this->get_chunk($start, ($numGlyphs * 2) + 2);
			$arr = \unpack('n*', $data);
			//--
			for($n=0; $n<=$numGlyphs; $n++) {
				$this->glyphPos[] = ($arr[$n+1] * 2);
			} //end for
			//--
		} elseif((int)$indexToLocFormat === 1) {
			//--
			$data = $this->get_chunk($start, ($numGlyphs * 4) + 4);
			$arr = \unpack('N*', $data);
			for($n=0; $n<=$numGlyphs; $n++) {
				$this->glyphPos[] = ($arr[$n+1]);
			} //end for
			//--
		} else {
			//--
			throw new \Exception('Unknown location table format '.$indexToLocFormat);
			return;
			//--
		} //end if else
		//--
	} //END FUNCTION


	private function getCMAP4(int $unicode_cmap_offset, array &$glyphToChar, array &$charToGlyph) : void {
		//--
		// CMAP Format 4
		//--
		$this->maxUniChar = 0;
		$this->seek($unicode_cmap_offset + 2);
		$length = $this->read_ushort();
		$limit = $unicode_cmap_offset + $length;
		$this->skip(2);
		//--
		$segCount = $this->read_ushort() / 2;
		$this->skip(6);
		$endCount = [];
		for($i=0; $i<$segCount; $i++) {
			$endCount[] = $this->read_ushort();
		} //end for
		$this->skip(2);
		$startCount = [];
		for($i=0; $i<$segCount; $i++) {
			$startCount[] = $this->read_ushort();
		} //end for
		$idDelta = [];
		for($i=0; $i<$segCount; $i++) {
			$idDelta[] = $this->read_short(); // ???? was unsigned short
		} //end for
		$idRangeOffset_start = $this->_pos;
		$idRangeOffset = [];
		for($i=0; $i<$segCount; $i++) {
			$idRangeOffset[] = $this->read_ushort();
		} //end for
		//--
		for($n=0; $n<$segCount; $n++) {
			$endpoint = ($endCount[$n] + 1);
			for($unichar=$startCount[$n]; $unichar<$endpoint; $unichar++) {
				if((int)$idRangeOffset[$n] === 0) {
					$glyph = ($unichar + $idDelta[$n]) & 0xFFFF;
				} else {
					$offset = ($unichar - $startCount[$n]) * 2 + $idRangeOffset[$n];
					$offset = $idRangeOffset_start + 2 * $n + $offset;
					if($offset >= $limit) {
						$glyph = 0;
					} else {
						$glyph = $this->get_ushort($offset);
						if((int)$glyph !== 0) {
							$glyph = ($glyph + $idDelta[$n]) & 0xFFFF;
						} //end if
					} //end if else
				} //end if else
				$charToGlyph[$unichar] = $glyph;
				if($unichar < 196608) {
					$this->maxUniChar = max($unichar,$this->maxUniChar);
				} //end if
				$glyphToChar[$glyph][] = $unichar;
			} //end for
		} //end for
		//--
	} // END FUNCTION


//	private function endTTFile(&$stm) {
	private function endTTFile() : string { // unixman
		//--
		// Put the TTF file together
		//--
		$stm = '';
		$numTables = (int) \count($this->otables);
		$searchRange = 1;
		$entrySelector = 0;
		//--
		while($searchRange * 2 <= $numTables) {
			$searchRange = $searchRange * 2;
			$entrySelector = $entrySelector + 1;
		} //end while
		//--
		$searchRange = $searchRange * 16;
		$rangeShift = $numTables * 16 - $searchRange;
		//-- Header
		if(self::TTF_MAC_HEADER === true) {
			$stm .= (string) \pack('Nnnnn', 0x74727565, $numTables, $searchRange, $entrySelector, $rangeShift); // Mac
		} else {
			$stm .= (string) \pack('Nnnnn', 0x00010000 , $numTables, $searchRange, $entrySelector, $rangeShift); // Windows
		} //end if else
		//-- Table directory
		$tables = $this->otables;
		//--
		\ksort ($tables);
		$offset = 12 + $numTables * 16;
		foreach($tables as $tag => $data) {
			if((string)$tag === 'head') {
				$head_start = $offset;
			} //end if
			$stm .= $tag;
			$checksum = $this->calcChecksum($data);
			$stm .= (string) \pack('nn', $checksum[0], $checksum[1]);
			$stm .= (string) \pack('NN', $offset, \strlen($data));
			$paddedLength = ((int)\strlen($data) + 3) &~ 3;
			$offset = $offset + $paddedLength;
		} //end foreach
		//-- Table data
		foreach($tables as $tag=>$data) {
			$data .= "\0\0\0";
			$stm .= (string) \substr((string)$data, 0, ((int)\strlen($data) &~ 3));
		} //end foreach
		//--
		$checksum = $this->calcChecksum($stm);
		$checksum = $this->sub32([ 0xB1B0, 0xAFBA ], $checksum);
		$chk = (string) \pack('nn', $checksum[0],$checksum[1]);
		$stm = $this->splice($stm,($head_start + 8),$chk);
		//--
		return (string) $stm;
		//--
	} //END FUNCTION


} //END CLASS


// #end
