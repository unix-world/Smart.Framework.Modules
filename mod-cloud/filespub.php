<?php
// Controller: Cloud/FilesPub
// Route: admin.php/page/cloud.filespub/~
...
if(!SmartAppInfo::TestIfModuleExists('mod-webdav')) {
	http_response_code(500);
	echo SmartComponents::http_message_500_internalerror('ERROR: Cloud Public Files requires Mod WebDAV ...');
	die('');
} //end if
//--
...
class SmartAppAdminController extends \SmartModExtLib\Webdav\ControllerAdmDavFs {
	...
	public function Run() {
		...
		if(SmartFileSystem::is_type_dir((string)$safe_user_path) !== true) {
			http_response_code(500);
			echo SmartComponents::http_message_500_internalerror('ERROR: WebDAV User Public Path does not exists ...');
			return;
		} //end if
		//--
		...
		if((string)ltrim((string)$this->RequestPathGet(), '/') != '') {
			$txt_lnk = 'Cloud.PublicFiles :: Home';
			$url_lnk = (string) SmartUtils::get_server_current_url().SmartUtils::get_server_current_script().'/page/cloud.filespub/~';
		} else {
			$txt_lnk = 'Cloud :: Home';
