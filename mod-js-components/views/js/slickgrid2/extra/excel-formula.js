 ...
 * 		* fixed max/min range:range formula
...
(function(root) {
		...
		var formula2JavaScript = excelFormulaUtilities.formula2JavaScript = function(formula) {
			...
			//Custom callback to format as js
			var functionStack = [];

			var tokRender = function(tokenStr, token, indent, linbreak) {

				var outstr = '',
				tokenString = tokenStr, // tokenString = (token.value.length === 0) ? '' : token.value.toString(),
				directConversionMap = {
					'=': 		'==',
					'<>': 		'!=',
					'ABS': 		'Math.abs',
...
//		console.log(currentFunctionOnStack.name.toUpperCase() + ' @ ' + outstr);
											break;
										// extra: added by unixman
										case 'min':
										case 'max':
										...
										case 'count':
											...
											if(RegExp(':','gi').test(tokenString)) {
												outstr = breakOutRanges(tokenString, '+');
												var regex = /([\+])/gi; // {{{SYNC-REGEX-EXCEL-COUNT}}}
												outstr = outstr.replace(regex, '*0+1+') + '*0+1'; // converts to 1+1+1+...+1
											} else {
												outstr = tokenString;
											} //end if else
											//console.log(currentFunctionOnStack.name.toUpperCase() + ' @ ' + outstr);
											break;
										case 'average': // sum-of-nums/count-of-nums
											// Create array for ranges
											var regex = /([\+])/gi; // {{{SYNC-REGEX-EXCEL-COUNT}}}
											if(RegExp(':','gi').test(tokenString)) {
												outstr = '(' + breakOutRanges(tokenString, '+');
												outstr += ')/' + outstr.replace(regex, '*0+1+') + '*0+1' + ')'; // converts to sum()/count()
											} else {
												outstr = tokenString;
											} //end if else
											//console.log(currentFunctionOnStack.name.toUpperCase() + ' @ ' + outstr);
			...
			var outFormula = String(theOutput).replace('==', '==='); // js uses === instead of == to strict compare
		//	console.log(outFormula);

			return String(outFormula);
