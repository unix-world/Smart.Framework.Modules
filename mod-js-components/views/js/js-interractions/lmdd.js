...
// (c) 2017 Yair Levy
// License: MIT
// https://github.com/supraniti/Lean-Mean-Drag-and-Drop

// (c) 2019 unix-world.org
// contains fixes by unixman r.20190227
...
var lmdd = (function () {
	...
	var options = {
		...
		positionDelay: false,
		dataMode: false,
		lmddend: null // by unixman: function(objPositions, elRef, isCloned){} || null
	};
	...
	function killEvent() { //end current drag event
		...
		if (status !== "dragStartTimeout" && status !== "waitDragStart") {
			...
			//-- unixman
			if(typeof scope.lmddOptions.lmddend == 'function') {
				//console.log(JSON.stringify(positions.originalContainer.id), JSON.stringify(positions.currentContainer.id));
				scope.lmddOptions.lmddend(positions, dragged, cloning); // dragged (is the element)
			} //end if
			//-- #
		}
