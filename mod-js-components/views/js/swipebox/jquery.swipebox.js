	...
	$.swipebox = function( elem, options ) {
		...
		var ui,
			defaults = {
				...
				queryStringData: {},
				toggleClassOnLoad: '',
				selector: null, // fix by unixman
				allowClose: true // option by unixman
			},

			plugin = this,
			elements = [], // slides array [ { href:'...', title:'...' }, ...],
			$elem,
			//selector = elem.selector, // fix by unixman
		...
		plugin.init = function() {
			...
			if ( Array.isArray( elem ) ) {
				...
				//$( document ).on( 'click', selector, function( event ) { // fix by unixman
				$( elem ).on( 'click', plugin.settings.selector, function( event ) {
					...
					if ( relVal && relVal !== '' && relVal !== 'nofollow' ) {
						...
						$elem = $( selector );
						*/
						$elem = $elem.filter( '[' + relType + '="' + relVal + '"]' ); // fix by unixman
					}
		...
		ui = {
			...
			keyboard : function () {
				...
				$( window ).on( 'keyup', function( event ) {
					...
					if ( event.keyCode === 37 ) {
						...
						if(plugin.settings.allowClose !== false) { // enhancement by unixman
							$this.closeSlide();
						} //end if
					}
				} );
			...
			openSlide : function ( index ) {
				...
				if ( plugin.settings.allowClose === false ) {
