	...
	dust.filters = {
		...
		*/
		...
		o: 	function(value) {
				var jsonObj = null;
				try {
					jsonObj = JSON.parse(value); // it MUST be JSON !
				} catch(err){
					jsonObj = null;
				} //end try catch
				value = JSON.stringify(jsonObj, null, null); // ensure no pretty print or json replacer is used
				...
				value = value.replace(/[\u0026]/g, '\\u0026');	// & 	JSON_HEX_AMP
		...
		vj: function(value) { // format js VAR
				value = String(value);
				value = value.replace(/[^a-zA-Z0-9_]/g, '');
				return String(value);
			},
