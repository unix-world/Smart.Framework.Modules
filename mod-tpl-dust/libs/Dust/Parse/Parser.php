...
class Parser {
	...
	// the commented code is a new feature developed by unixman to allow a topic like {> sub-template="{Tpl_Dir__}partial.dust" /} :: {{{SYNC-DUST-UXM-TPL-INCLUDE-FEATURE-SUBTEMPLATE}}}
		if ($ctx->peek() != '{') return null;
		$type = $ctx->peek(2);
		if ($type != '>' && $type != '+') return null;
		$partial = new Ast\Partial($ctx->offset);
		$ctx->offset += 2;
	//	$save_offset = $ctx->offset;
		$partial->type = $type;
		$ctx->skipWhitespace(); // fix by unixman to support pre/post space in partials include expression
		$partial->key = $this->parseKey($ctx);
		if($partial->key == null) {
			$partial->inline = $this->parseInline($ctx);
	//	} else {
	//		$ctx->offset = $save_offset;
		}
		$partial->context = $this->parseContext($ctx);
		$partial->parameters = $this->parseParameters($ctx);
	/*
		$parameters = $this->parseParameters($ctx);
		$partial->parameters = array();
		if(is_array($parameters)) {
			if(count($parameters) > 0) {
				for($i=0; $i<count($parameters); $i++) {
					if(is_a($parameters[$i], '\\Dust\\Ast\\InlineParameter')) {
						if((string)$parameters[$i]->key == 'sub-template') { // {{{SYNC-UXM-FIX-DUST-SUBTEMPLATE}}}
							$partial->inline = $parameters[$i]->value;
						} else {
							$partial->parameters[] = $parameters[$i];
						}
					}
				}
			}
		...
		$parameters = null;
		//print_r($partial->inline); print_r($partial->parameters); die();
	*/
		$ctx->skipWhitespace();
	...
	//	$comment->contents = substr($ctx->str, $ctx->offset + 2, $endIndex - $ctx->offset - 4);
		$comment->contents = trim(substr($ctx->str, $ctx->offset + 2, $endIndex - $ctx->offset - 2)); // fix by unixman
