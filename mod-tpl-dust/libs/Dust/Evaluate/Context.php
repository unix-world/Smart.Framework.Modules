...
class Context {
	...
	public function push($head, $index = null, $length = null, $iterationCount = null) {
		...
		if($index !== null) {
			$state->params['$idx'] = (string) $index; // current index _-KEY-
		}
		if($length !== null) {
			$state->params['$len'] = (int) $length; // -_MAXSIZE_-
			$state->params['$cnt'] = (int) ($length ? ($length - 1) : 0); // added by unixman _-MAXCOUNT-_
		}
		if($iterationCount !== null) {
			$state->params['$iter'] = (int) $iterationCount; // _-ITERATOR-_
			$state->params['$icnt'] = (int) $iterationCount + 1; // added by unixman -_INDEX_-
		}
