...
class Evaluator {
	...
	public function evaluateSection(Ast\Section $section, Context $ctx, Chunk $chunk) {
		...
		if ($section->type == '+') {
			...
			$dstHelpers = (array) $this->dust->getHelpers();
			if (!isset($dstHelpers[$section->identifier->key])) {
				$this->error($section->identifier, 'Unable to find helper "' . $section->identifier->key . '"');
			}
			$helper = $dstHelpers[$section->identifier->key];
	...
	public function evaluateReference(Ast\Reference $ref, Context $ctx, Chunk $chunk) {
		...
		if (!$this->isEmpty($resolved)) {
			...
			if (empty($ref->filters)) {
				//default filters
				$dstAFilters = (array) $this->dust->getAutomaticFilters();
				$resolved = array_reduce($dstAFilters, function($prev, Filter\Filter $filter) {
					return $filter->apply($prev);
				}, $resolved);
			} else {
				//apply filters in order...
				$resolved = array_reduce($ref->filters, function($prev, Ast\Filter $curr) {
					$dstFilters = (array) $this->dust->getFilters();
					if(array_key_exists($curr->key, $dstFilters)) {
						$filter = $dstFilters[$curr->key];
						return $filter->apply($prev);
					} else {
						\Smart::log_warning(__METHOD__.'() :: Invalid TPL Filter: '.$curr->key);
						return $prev;
					}
				}, $resolved);
			}
