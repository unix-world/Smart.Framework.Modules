...
final class SmartAppTaskController extends SmartAbstractAppController {
	...
	//	$semaphores[] = 'load:searchterm-highlight-js';
		...
		$html = (string) \SmartModExtLib\Docs\OptimizationUtils::renderDocMarkdown(
			(string) $source,
			'', // '<validate:html:tidy:required>' ; perhaps better to disable tidy ... takes double as time, especially on big html ; ex: realm=d3-4&key=36 ; the html from which this markdown renders was already pre-filtered with tidy and the markdown was generated by a trusted PHP code (League HtmlToMarkdown with unixman's fixes !)
