...
final class SmartAppTaskController extends SmartAbstractAppController {
	...
	private const DOCS_FILE = 'db.json';
	private const REQ_MAX_MEMORY_SIZE = '512M';
	...
	public function Initialize() {
		...
		if((string)ini_get('memory_limit') !== (string)self::REQ_MAX_MEMORY_SIZE) {
			$this->err = 'Failed to set PHP.INI memory_limit as: '.(string)self::REQ_MAX_MEMORY_SIZE;
			return;
		} //end if
		//--
	} //END FUNCTION
	...
	//	$semaphores[] = 'load:searchterm-highlight-js';
				...
				case 'render-markdown':
					...
					$this->PageViewSetVars([
						...
						'main' 				=> SmartMarkersTemplating::render_file_template(
							$this->ControllerGetParam('module-view-path').'tasks/'.$this->ControllerGetParam('action').'-render-markdown'.'.mtpl.htm',
							[
		...
		foreach($arr as $key => $val) {
			if(
				...
				$key_id = (string) $key;
				$source = (string) (new SmartHtmlParser((string)$val, true, 2, false))->get_clean_html(); // prefer Tidy here, it is more safe for untrusted inputs ...
				break;
			} //end if
			$index++;
		} //end foreach
