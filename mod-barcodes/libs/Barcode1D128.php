...
final class Barcode1D128 {
	...
	private const FNC_A = [ 241 => 102, 242 => 97, 243 => 96, 244 => 101 ];

	private const FNC_B = [ 241 => 102, 242 => 97, 243 => 96, 244 => 100 ];


	public function __construct(?string $code, ?string $type='B') {
		//--
	...
	public function getBarcodeArray() : array { // barcode_c128()
		...
		switch((string)\strtoupper($type)) {
			case 'A':  // MODE A
				...
				for($i=0; $i<$len; ++$i) {
					...
					if(($char_id >= 241) AND ($char_id <= 244)) {
						$code_data[] = (int) self::FNC_A[$char_id];
					} elseif(($char_id >= 0) AND ($char_id <= 95)) {
						$code_data[] = (int) \strpos($keys_a, $char);
					} else {
						return [];
					} //end if else
				} //end for
			...
			case 'C': // MODE C
				...
				for($i=0; $i<$len; $i+=2) {
					$chrnum = (string) $code[$i].$code[$i+1];
					if(\intval(\preg_match('/([0-9]{2})/', (string)$chrnum)) > 0) {
			...
			default:
				...
				for($i = 0; $i < $len; ++$i) {
					...
					$char_id = (int) \ord($char);
					if(($char_id >= 241) AND ($char_id <= 244)) {
						$code_data[] = (int) self::FNC_B[$char_id];
					} elseif(($char_id >= 32) AND ($char_id <= 127)) {
						$code_data[] = (int) \strpos($keys_b, $char);
					} else {
						return [];
					} //end if else
				} //end for
		...
		foreach($code_data as $u => $val) {
			$seq = (string) self::CHAR[(string)$val];
			for($j=0; $j<6; ++$j) {
				$t = false; // space
				if(($j % 2) == 0) {
					$t = true; // bar
				...
				$w = (int) \intval($seq[$j]);
				$bararray['bcode'][] = [ 't' => (int)$t, 'w' => (int)$w, 'h' => 1, 'p' => 0 ];
				$bararray['maxw'] += (int) $w;
			} //end for
		} //end foreach
		//--
