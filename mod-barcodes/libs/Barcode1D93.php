...
final class Barcode1D93 {
	...
	public function __construct(?string $code, bool $extended=true, bool $checksum=true) {
		//--
	...
	public function getBarcodeArray() : array {
		...
		//--
		for($i=0 ; $i<$clen; ++$i) {
			if((int)\ord((string)$code[$i]) > 127) {
				return [];
			} //end if
			$code_ext .= (string) $encode[(string)$code[$i]];
		} //end for
		//-- checksum
		$code_ext .= (string) $this->checksum((string)$code_ext);
		//--
		...
		//--
		$bararray['bcode'][$k] = [ 't' => 1, 'w' => 1, 'h' => 1, 'p' => 0 ];
		$bararray['maxw'] += 1;
		//--
		++$k;
		//--
	...
	private function checksum(string $code) : string {
		...
		$code = (string) \strtr($code, \chr(128).\chr(131).\chr(129).\chr(130), '<=>?');
		$len = (int) \strlen($code);
		...
		//--
		$check %= 47;
		$c = (string) self::CHECKSUM[(string)$check];
		$code .= (string) $c;
		...
		//--
		$check %= 47;
		$k = (string) self::CHECKSUM[(string)$check];
		$checksum = (string) $c.$k;
		//-- restore special characters
		$checksum = (string) \strtr($checksum, '<=>?', \chr(128).\chr(131).\chr(129).\chr(130));
		//--
