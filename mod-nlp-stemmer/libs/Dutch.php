<?php
// Class: \SmartModExtLib\NlpStemmer\Dutch
 ...
 *
 */
final class Dutch extends \SmartModExtLib\NlpStemmer\Stem {
	...
	public function stem($word)
	{
		...
		$this->word = \SmartModExtLib\NlpStemmer\Utf8::str_replace(
	...
	private function hasValidEnEnding($word)
	{
		...
		$threeLastLetters = \SmartModExtLib\NlpStemmer\Utf8::substr($word, -3, 3);
	...
	private function step2()
	{
		if ( ($position = $this->search(array('e'))) !== false) {
			if ($this->inR1($position)) {
				$letter = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -2, 1);
	...
	private function step3a()
	{
		if ( ($position = $this->search(array('heid'))) !== false) {
			if ($this->inR2($position)) {
				$letter = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -5, 1);
				if ($letter !== 'c') {
					$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, $position);
		...
		if ( ($position = $this->search(array('end', 'ing'))) !== false) {
			if ($this->inR2($position)) {
				...
				if ( ($position2 = $this->searchIfInR2(array('ig'))) !== false) {
					$letter = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -3, 1);
					if ($letter !== 'e') {
						$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, $position2);
					}
				} else {
					$this->unDoubling();
				}
	...
	private function step4()
	{
		...
		$d = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -1, 1);
		if (in_array($d, array_merge(self::$vowels, array('I')))) {
			return false;
		}

		// V is double a, e, o or u
		$v = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -3, 2);
		if (!in_array($v, array('aa', 'ee', 'oo', 'uu'))) {
			return false;
		}
		$singleV = \SmartModExtLib\NlpStemmer\Utf8::substr($v, 0, 1);

		// C is a non-vowel
		$c = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, -4, 1);
		if (in_array($c, self::$vowels)) {
			return false;
		}

		$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, -4);
	...
	{
		$this->word = \SmartModExtLib\NlpStemmer\Utf8::str_replace(array('I', 'Y'), array('i', 'y'), $this->word);
	}
