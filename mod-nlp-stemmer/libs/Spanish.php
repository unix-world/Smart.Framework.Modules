<?php
// Class: \SmartModExtLib\NlpStemmer\Spanish
 ...
 *
 */
final class Spanish extends \SmartModExtLib\NlpStemmer\Stem {
	...
	private function step0()
	{
		if ( ($position = $this->searchIfInRv(array('selas', 'selos', 'las', 'los', 'les', 'nos', 'selo', 'sela', 'me', 'se', 'la', 'le', 'lo' ))) != false) {
			$suffixe = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, $position);
			...
			if ( ($position2 = $this->searchIfInRv($a)) !== false) {
				$suffixe2 = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, $position2);
				$suffixe2 = \SmartModExtLib\NlpStemmer\Utf8::deaccent($suffixe2, -1);
			...
			if ( ($position2 = $this->searchIfInRv(array('yendo' . $suffixe))) != false) {
				$before = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, ($position2-1), 1);
		...
		if ( ($position = $this->search(array('amente'))) != false) {
			...
			if ( ($position2 = $this->searchIfInR2(array('iv'))) !== false) {
				...
				if ( ($position3 = $this->searchIfInR2(array('at'))) !== false) {
					$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, $position3);
				}

			// if preceded by os, ic or ad, delete if in R2
			} elseif ( ($position4 = $this->searchIfInR2(array('os', 'ic', 'ad'))) != false) {
				$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, $position4);
			}
			return true;
		}
	...
	private function step2a()
	{
		...
		if ( ($position = $this->searchIfInRv(array(
			...
			$before = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, ($position-1), 1);
			if ( (isset($before)) && ($before == 'u') ) {
				$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, $position);
				return true;
			}
	...
	{
		$this->word = \SmartModExtLib\NlpStemmer\Utf8::str_replace(array('á', 'í', 'ó', 'é', 'ú'), array('a', 'i', 'o', 'e', 'u'), $this->word);
	}
