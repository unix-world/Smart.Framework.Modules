<?php
// Class: \SmartModExtLib\NlpStemmer\English
 ...
 *
 */
final class English extends \SmartModExtLib\NlpStemmer\Stem {
	...
	public function stem(string $word) {
		...
		if (\SmartModExtLib\NlpStemmer\Utf8::strlen($word) < 3) {
			return $word;
		}
		...
		if ($first == "'") {
			$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 1);
		}
			...
			for ($i=0; $i<$position; $i++) {
				...
				if (in_array($letter, self::$vowels)) {
					...
					if ($this->search(array('at', 'bl', 'iz')) !== false) {
						...
						$this->word = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 0, ($position2+1));
	...
	private function step2()
	{
		...
		if ( ($position = $this->search(array('li'))) !== false) {
			...
			if ($this->inR1($position)) {
				// a letter for you
				$letter = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, ($position-1), 1);
	...
	{
		$this->word = \SmartModExtLib\NlpStemmer\Utf8::str_replace('Y', 'y', $this->word);
	}

	private function exceptionR1()
	{
		if (\SmartModExtLib\NlpStemmer\Utf8::strpos($this->word, 'gener') === 0) {
			$this->r1 = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 5);
			$this->r1Index = 5;

		} elseif (\SmartModExtLib\NlpStemmer\Utf8::strpos($this->word, 'commun') === 0) {
			$this->r1 = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 6);
			$this->r1Index = 6;

		} elseif (\SmartModExtLib\NlpStemmer\Utf8::strpos($this->word, 'arsen') === 0) {
			$this->r1 = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, 5);
			$this->r1Index = 5;
		}
	}
	...
	private function searchShortSyllabe($from, $nbLetters)
	{
		...
		$first = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, $from, 1);
		$second = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, ($from+1), 1);
		...
		$third = \SmartModExtLib\NlpStemmer\Utf8::substr($this->word, ($from+2), 1);
