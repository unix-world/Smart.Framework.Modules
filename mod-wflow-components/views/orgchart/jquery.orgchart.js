/*
 * jQuery OrgChart Plugin v.3.1.4
 ...
 *
 * contains fixes + extended features by unixman
 */
'use strict';
...
(function (factory) {
	...
	OrgChart.prototype = {
		...
		buildJsonDS: function ($li) {
			var that = this;
			// fix by unixman to better parse ul/li
		//	var subObj = {
		//		'name': $li.contents().eq(0).text().trim(),
		//		'relationship': ($li.parent().parent().is('li') ? '1': '0') + ($li.siblings('li').length ? 1: 0) + ($li.children('ul').length ? 1 : 0)
		//	};
			var subObj = {
				'name': (typeof($li.eq(0).attr('data-name')) != 'undefined') ? ($li.eq(0).attr('data-name') || '').trim() : $li.contents().eq(0).text().trim(),
				'title': ($li.eq(0).attr('title') || '').trim(),
				'href': ($li.contents().eq(0).first().attr('href') || '').trim(),
				'relationship': ($li.parent().parent().is('li') ? '1': '0') + ($li.siblings('li').length ? 1: 0) + ($li.children('ul').length ? 1 : 0)
			};
			// #end fix
		...
		//		.addClass('node ' + (data.className || '') +  (level > opts.visibleLevel ? ' slide-up' : ''));
			var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + $('<div></div>').text(data[opts.nodeId] || '').html() + '"' : '') + (data.parentId ? ' data-parent="' + $('<div></div>').text(data.parentId || '').html() + '"' : '') + '>')
				.addClass('node ' + $('<div></div>').text(data.className || '').html() +  (level > opts.visibleLevel ? ' slide-up' : ''));
			//-- #end fix
			...
			//		.append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
				$nodeDiv.append('<div class="title">' + $('<div></div>').text(data[opts.nodeTitle] || '').html() + '</div>')
					.append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + $('<div></div>').text(data[opts.nodeContent] || '').html() + '</div>' : '');
				//-- #end fix
			}
			//
