...
'use strict';
	...
	Canvas.prototype._fitViewport = function(center) {
		...
		if (inner.x >= 0 &&
			...
			newViewbox = { // fix by unixman: make all integers
				x: inner.x + Math.floor(center ? inner.width / 2 - outer.width / newScale / 2 : 0),
				y: inner.y + Math.floor(center ? inner.height / 2 - outer.height / newScale / 2 : 0),
				width: Math.floor(outer.width / newScale),
				height: Math.floor(outer.height / newScale)
			};
		}
	...
	var UNIXWORLD_LOGO = '<svg xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" version="1.1" width="32" height="32" viewBox="0 0 72.248893 72.248893" id="sign-info-svg" xml:space="preserve" style="fill-rule:evenodd"><defs id="defs4"><style type="text/css" id="style6" /></defs><g id="text3781" style="fill:#ffffff;fill-opacity:1;stroke:none;"><g id="g3009"><path d="m 62.930573,36.124446 a 26.806128,26.806128 0 1 1 -53.6122535,0 26.806128,26.806128 0 1 1 53.6122535,0 z" id="path3011" style="fill:#778899;fill-opacity:1;fill-rule:evenodd;stroke:#778899;stroke-width:14.12108517;stroke-opacity:1" /><path d="m 40.052979,29.865886 -10.40384,0 0,1.30048 c 2.384211,0.487679 2.871893,0.975362 2.871893,3.197013 l 0,15.93088 c 0,2.221652 -0.379309,2.655148 -2.871893,3.251201 l 0,1.30048 12.950614,0 0,-1.30048 c -1.896532,-0.270933 -2.546774,-1.029549 -2.546774,-3.088641 l 0,-20.590933 M 36.259912,17.402952 c -2.330024,0 -4.22656,1.896535 -4.22656,4.172373 0,2.438398 1.788163,4.22656 4.172374,4.22656 2.384211,0 4.22656,-1.788162 4.22656,-4.172373 0,-2.330025 -1.842349,-4.22656 -4.172374,-4.22656" id="path3006" style="fill:#ffffff;" /></g></g></svg>';

	/**
	 * The logo code must not be removed or changed.
	...
	var BPMNIO_IMG = '<img width="52" height="52" src="' + 'data:image/svg+xml,' + encodeURIComponent(BPMNIO_LOGO_SVG) + '" style="opacity:0.85;" />';
		...
		'<div class="bjs-powered-by-lightbox" style="' + LIGHTBOX_STYLES + '">' +
			...
			'<div class="notice" style="' + NOTICE_STYLES + '">' +
				'<span style="float: left; margin-right: 10px">' +
					BPMNIO_IMG +
				'</span>' +
				'Web-based tooling for BPMN, DMN and CMMN diagrams powered by <a style="color:#555555; text-decoration-style: dotted;" href="http://bpmn.io" target="_blank">bpmn.io</a>' +
			'</div>' +
		'</div>';
	...
	//-- unixman
	/**
	 * Parse and render a BPMN 2.0 diagram JSON.
	 *
	 ...
	 *
	 * @param {String} json the BPMN json
	 * @param {Function} [done] invoked with (err, warnings=[])
	 */
	Viewer.prototype.importJSON = function(json, done) { // {{{SYNC-BPMN-UXM-IMPORT-DATA}}}
		...
		if(!(json.data.hasOwnProperty('bpmnXML'))) {
			return;
		} //end if
		this.importXML(json.data.bpmnXML, done, json.data.bpmnProps);
	};
	//--
	 ...
	 * @param {Function} [done] invoked with (err, warnings=[])
	 * @param {Object} diagramProps (by unixman)
	 */
	Viewer.prototype.importXML = function(xml, done, diagramProps) { // {{{SYNC-BPMN-UXM-IMPORT-DATA}}}
		// done is optional
		done = done || function() {};
		var self = this;
		...
		this._moddle.fromXML(xml, 'bpmn:Definitions', function(err, definitions, context) {
			...
			//-- unixman: import annotations {{{SYNC-BPMN-UXM-ANNOTATIONS}}}
			function restoreReadOnlyAnnotations() {
				var _canvas = self.get('canvas');
				var _overlays = self.get('overlays');
				if(typeof diagramProps == 'object') {
					var theNote = '';
					var overlayHtml = '';
					var id = '';
					for(var key in diagramProps) {
						if(key && diagramProps[key]) {
							theNote = diagramProps[key]['note'];
							if(theNote) {
								overlayHtml = createReadOnlyAnnotation(String(key), String(theNote), _canvas);
								id = _overlays.add(String(key), 'note', { // attach an overlay to a node
									position: {
										bottom: 0,
										right: 0
									},
								...
								if(id) {
									_canvas.addMarker(String(key), 'djs-xtra-annotation-note'); // add marker
								} //end if
							} //end if
						} //end if
					} //end for
			...
			self.importDefinitions(definitions, function(err, importWarnings) {
				...
				restoreReadOnlyAnnotations(); // unixman
			});
		});
	};
	...
	function addUxmLogo(container) {
		var linkMarkup =
			'<a href="#" ' +
				'class="bjs-uxm-powered-by" ' +
				'title="BPMN Viewer, License: GPLv3\nDeveloped by unix-world.org\nOriginal code by bpmn.io" ' +
				'style="position: absolute; bottom: 75px; right: 25px; z-index: 101; cursor:help;">' +
				'<img width="32" height="32" src="' + 'data:image/svg+xml,' + encodeURIComponent(UNIXWORLD_LOGO) + '" style="opacity:0.85;">' +
			'</a>';
		var linkElement = domify(linkMarkup);
		container.appendChild(linkElement);
		componentEvent.bind(linkElement, 'click', function(event$$1) {
