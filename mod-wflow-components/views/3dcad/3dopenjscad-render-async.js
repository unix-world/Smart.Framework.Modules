...
// 3dopenjscad-render-async.js
// 3D Js CAD - Async Renderer (Worker)
// (c) 2017-2019 unix-world.org
// License: GPLv3
// Fixes by unixman r.20190202

// based on jscad-worker.js
...
OpenJsCad.createJscadWorker = function(fullurl, script, callback) {
	...
	//console.log('Render Async (Worker): ' + fullurl);
	...
	if(e.data instanceof Object) {
		...
		if(data.cmd == 'render') {
			...
			if(typeof(main) == 'function') {
				...
				if(evObjects) {
					//console.log('EvObjects: ' + evObjects);
					eval(evObjects);
				}
				//--#
				var results;
				try {
					results = main(data.parameters);
				} catch(err){
					r.err = String(err);
				/*	if(debug) {
						if(err.stack) {
