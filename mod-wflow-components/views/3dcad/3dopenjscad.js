...
// 		* openJsCadPath as option
// 		* openJsCadTextUpdate as option
// 		* openJsCadTextInstantUpdate as option
// 		* openJsCadTextExport as option
// 		* openJsCadFxExport as option
// 		* prefix all css styles used by openscad

// == based on openjscad.js, originally written by Joost Nieuwenhuijse (MIT License)
...
// see the various methods below on how to change these
	...
	this.solid = {
		...
		color:   [250/255,128/255,114/255,1], // default color: salmon (prev was: [1,.4,1,1]), by unixman
	};
...
OpenJsCad.AlertUserOfUncaughtExceptions = function() {
	window.onerror = function(message, url, line) {
		...
		if(typeof document == 'object') {
			var e = document.getElementById("openscadAreaError");
			if (e !== null) {
...
// the default options
	this.opts = {
		debug: false,
		openJsCadPath: '', // the path to the libs
		openJsCadTextUpdate: 'Update', // text for the update button (params area)
		openJsCadTextInstantUpdate: 'Instant Update', // text for the instant update checkbox (params area)
		openJsCadTextExport: 'Export', // text for export button
		openJsCadFxExport: null, // function to export
		//--
		libraries: ['geometry.js','formats.js','objects.js'], //,'3dopenjscad.js'],
		useAsync: false, // use openjscad-worker
		useSync:  true,  // use openjscad-function
		viewer: {},
	};
...
OpenJsCad.Processor.prototype = {
	createElements: function() {
		...
		this.openscadAreaError = this.containerdiv.parentElement.querySelector("div#openscadAreaError");
		if (!this.openscadAreaError) {
			this.openscadAreaError = document.createElement("div");
			this.openscadAreaError.id = 'openscadAreaError';
			this.containerdiv.parentElement.appendChild(this.openscadAreaError);
		}
		this.errorpre = document.createElement("pre");
		this.openscadAreaError.appendChild(this.errorpre);

		this.openscadAreaStatus = this.containerdiv.parentElement.querySelector("div#openscadAreaStatus");
		if (!this.openscadAreaStatus) {
			this.openscadAreaStatus = document.createElement("div");
			this.openscadAreaStatus.id = "openscadAreaStatus";
			this.containerdiv.parentElement.appendChild(this.openscadAreaStatus);
		}
		this.openscadAreaFormats = this.containerdiv.parentElement.querySelector('div#openscadAreaFormats');
		this.statusdiv = document.createElement("div");
		this.statusdiv.id = 'statusdiv';
		this.statusbuttons = document.createElement("span");
		this.statusbuttons.id = 'statusbuttons';
		this.openscadAreaStatus.appendChild(this.statusdiv);
		this.openscadAreaFormats.appendChild(this.statusbuttons);
		...
		//--
		this.openscadAreaParams = this.containerdiv.parentElement.querySelector('div#openscadAreaParams');
		if (!this.openscadAreaParams) {
			this.openscadAreaParams = document.createElement("div");
			this.openscadAreaParams.id = "openscadAreaParams";
			this.containerdiv.parentElement.appendChild(this.openscadAreaParams);
		}
		this.openscadTblParams = document.createElement("table");
		this.openscadTblParams.className = "openscadTblParams";
		this.openscadAreaParams.appendChild(this.openscadTblParams);

		element = this.openscadAreaParams.querySelector("button#openscadButtonUpdate");
		if (element === null) {
			element = document.createElement("button");
			element.innerText = this.opts.openJsCadTextUpdate ? String(this.opts.openJsCadTextUpdate) : '@';
			element.id = "openscadButtonUpdate";
		}
		element.onclick = function(e) {
			that.rebuildSolid();
		};
		this.openscadAreaParams.appendChild(element);

		// implementing instantUpdate
		var instantUpdateCheckbox = document.createElement("input");
		instantUpdateCheckbox.type = "checkbox";
		instantUpdateCheckbox.id = "openscadChkBoxInstUpd";
		this.openscadAreaParams.appendChild(instantUpdateCheckbox);

		element = document.getElementById("openscadLblInstUpd");
		if (element === null) {
			element = document.createElement("label");
			element.innerText = this.opts.openJsCadTextInstantUpdate ? String(this.opts.openJsCadTextInstantUpdate) : '!';
			element.id = "openscadLblInstUpd";
		}
	...
	updateDownloadLink: function() {
		...
		this.generateOutputFileButton.innerText = this.opts.openJsCadTextExport + ' ' + ext.toUpperCase();
	},
	...
	enableItems: function() {
		...
		this.openscadAreaParams.style.display = (this.paramControls.length > 0)? "inline-block":"none";     // was 'block'
		this.openscadAreaError.style.display = this.hasError? "block":"none";
		this.openscadAreaStatus.style.display = this.hasError? "none":"block";
	...
	setJsCad: function(script, filename) {
		if(!filename) {
			console.error('OpenJScad: No File to Render ...');
			return;
		}
		this.abort();
		this.paramDefinitions = [];
	...
	getFullScript: function()
	{
		...
		*/
		script += '\n';
	// add exports
		//-- {{{SYNC-OPENSCAD-EXPORTS}}}
		var vObjList = String(OpenSCad.getExportedObjectsVarList());
		if(vObjList) {
			//console.log('ExpVars: ' + vObjList);
			script += String(vObjList);
		}
	// add the main script
		script += this.script;
		return script;
	...
	rebuildSolidAsync: function(baseurl)
	{
		var parameters = this.getParamValues();
		var script     = this.getFullScript();
		if(!window.Worker) {
			throw new Error("Worker threads are unsupported.");
		}
		//console.log('have Workers');
		//--
		// create the worker
		var that = this;
		var t0 = performance.now();
		that.state = 1; // processing
		that.worker = OpenJsCad.createJscadWorker(String(baseurl + this.filename), script,
		// handle the results
			function(err, objs) {
				that.worker = null;
				if(err) {
					that.setError(err);
					that.setStatus('(Async/Worker) Render ERROR ...');
					that.state = 3; // incomplete
				} else {
					that.setCurrentObjects(objs);
					var t1 = performance.now();
					that.setStatus('(Async/Worker) Render Completed in ' + (t1 - t0) + 'ms ...');
	...
	// pass the libraries to the worker for import
		var libraries = this.opts.libraries.map( function(l) {
			return String(this.opts.openJsCadPath) + String(l); // Fix by unixman
		}, this);
	// start the worker
		that.worker.postMessage({cmd: "render", parameters: parameters, libraries: libraries});
	},

	rebuildSolidSync: function(baseurl)
	{
		//--
		var parameters = this.getParamValues();
		//-- {{{SYNC-OPENSCAD-EXPORTS}}}
		...
		var evObjects = String(OpenSCad.getExportedObjectsEvCode());
		if(evObjects) {
			theScript += '\n' + String(evObjects) + '\n';
		}
		theScript += '\n' + String(this.script) + '\n';
		...
		try {
			...
			var func = OpenJsCad.createJscadFunction(String(baseurl + this.filename), theScript);
			var objs = func(parameters);
			this.setCurrentObjects(objs);
			var t1 = performance.now();
			this.setStatus('(Sync/Fx) Render Completed in ' + (t1 - t0) + 'ms ...');
		...
		/*	if(this.opts.debug) {
			...
			this.setStatus('(Sync/Fx) Render ERROR ...');
			this.state = 3; // incomplete
		}
		this.enableItems();
		//--
	...
	{
		//--
		var baseurl = String(document.location.href); // this is for sub-files from jscad files
		baseurl = baseurl.replace(/#.*$/,''); // remove remote URL
		baseurl = baseurl.replace(/\?.*$/,''); // remove parameters
		...
		if(this.opts.useAsync) {
			if(!window.Worker) {
				console.warn('WARNING: Cannot use the Async (Worker) Renderer, ... not supported by the browser !');
				this.setStatus('*** Rendering (Sync): Please wait ' + this.getLoadingImg());
				this.rebuildSolidSync(baseurl);
				return;
			}
			this.setStatus('Rendering (Async): Please wait ' + this.getLoadingImg());
			try {
				this.rebuildSolidAsync(baseurl);
				return;
			} catch(err) {
			//	if(!this.opts.useSync) {
				...
				/*	if(this.opts.debug) {
					...
					this.setError(errtxt);
					this.setStatus('(Async/Worker) Render ERROR !');
					this.state = 3; // incomplete
					this.enableItems();
			//	}
			}
	...
	// convert based on the given format
		...
		switch(format) {
			...
			case 'svg':
				...
				break;
			case 'jscad':
				blob = String(this.script); // new Blob([this.script], {type: formatInfo.mimetype });
				break;
			case 'json':
				blob = object.toJSON();
				break;
	...
	formatInfo: function(format) {
		if ( this.formats === null ) {
			this.formats = {
					...
					json:  { displayName: "JSON", extension: "json", mimetype: "text/json", convertCSG: true, convertCAG: true },
					stla:  { displayName: "STL (A)", extension: "stl", mimetype: "application/sla", convertCSG: true, convertCAG: false },
					svg:   { displayName: "SVG (2D)", extension: "svg", mimetype: "image/svg+xml", convertCSG: false, convertCAG: true },
	...
	generateOutputFileBlobUrl: function() {
		if(typeof this.opts.openJsCadFxExport != 'function') {
			alert('NOTICE: No Export Function has been implemented ...');
			return;
		}
		//console.log("Trying download via BLOB URL");
		var data = this.currentObjectsToBlob();
		var info = this.selectedFormatInfo();
		var ext  = String(info.extension);
		var mime = String(info.mimetype);
		//console.log(JSON.stringify(String(data,null,2)), ext, mime);
		this.opts.openJsCadFxExport(data, ext, mime);
		return false; // don't handle click again
	},
	...
	createControl: function(definition) {
		var control_list = [
			...
			{type: "int"     , control: "text"    , required: ["index","type","name"], initial: 0},
			{type: "float"   , control: "text"    , required: ["index","type","name"], initial: 0.0},
			{type: "number"  , control: "text"    , required: ["index","type","name"], initial: 0.0},
	...
	createParamControls: function() {
		this.openscadTblParams.innerHTML = "";
		this.paramControls = [];
		...
		// add the appropriate element to the table
			...
			if(type == "group") {
				...
				control.onchange = function(e) {
					...
					if(document.getElementById("openscadChkBoxInstUpd").checked==true) {
						that.rebuildSolid();
					}
				};
			...
			this.openscadTblParams.appendChild(tr);
		}
	},
};
