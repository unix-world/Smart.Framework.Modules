...
// (c) 2008 Kenneth Kufluk http://kenneth.kufluk.com/
// License: MIT
...
(function ($) {
	...
	Node = function (obj, name, parent, opts) {
		...
		this.el = $('<a href="' + (this.href ? this.href : '#') + '">' + this.name + '</a>').addClass('node');
		$('#' + this.options.elemID).prepend(this.el); // unixman
	...
	//	this.color = (this.obj.activeNode.parent === this.start || this.obj.activeNode.parent === this.end) ? "red" : "blue";
		...
		var path_str = 'M' + (this.start.x - this.options.mapArea.left) + ' ' + (this.start.y - this.options.mapArea.top) + 'L' + (this.end.x - this.options.mapArea.left) + ' ' + (this.end.y - this.options.mapArea.top);
		var path_opts = {
			'stroke': this.strokeStyle,
			'opacity': 0.2,
			'stroke-width': '5px', 	// raw: raphael
			'strokeWidth': '5' 		// camelcase: snapsvg, fabricjs
		};
		if(this.options.canvasRenderer == 'raphael') {
			this.obj.canvas.path(path_str).attr(path_opts);
		} else if(this.options.canvasRenderer == 'snapsvg') {
			this.obj.canvas.path({path:path_str, stroke:path_opts.stroke, opacity:path_opts.opacity, strokeWidth:path_opts.strokeWidth});
		} else if(this.options.canvasRenderer == 'fabricjs') {
			this.obj.canvas.add(new fabric.Path(path_str, path_opts));
		} //end if else
	};
	...
	*/
	...
	$.fn.mindmap = function (options) {
		...
		options = $.extend({
			canvasRenderer: 'raphael', // raphael | snapsvg | fabricjs
			canvasError: 'console.error', // alert
			attract: 15,
			repulse: 6,
			damping: 0.55,
			timeperiod: 10,
		...
		return this.each(function() {
			...
			// create canvas
			if(options.canvasRenderer == 'raphael') {
				this.canvas = Raphael(options.mapArea.left, options.mapArea.top, options.mapArea.x, options.mapArea.y);
			//	$('#' + options.elemID).prepend('<svg id="' + options.elemID + '-canvas' + '"></svg>');
			//	$('#' + options.elemID + '-canvas').css({ 'position':'absolute', top:'0px', left:'0px', width:options.mapArea.x+'px', height:options.mapArea.y+'px' });
			//	this.canvas = Raphael($('#' + options.elemID + '-canvas'), options.mapArea.x, options.mapArea.y);
			} else if(options.canvasRenderer == 'snapsvg') {
				$('#' + options.elemID).prepend('<svg id="' + options.elemID + '-canvas' + '"></svg>');
				$('#' + options.elemID + '-canvas').css({ 'position':'absolute', top:'0px', left:'0px', width:options.mapArea.x+'px', height:options.mapArea.y+'px' });
				this.canvas = Snap('#' + options.elemID + '-canvas');
			} else if(options.canvasRenderer == 'fabricjs') {
				$('#' + options.elemID).prepend('<canvas id="' + options.elemID + '-canvas' + '"></canvas>');
				$('#' + options.elemID + '-canvas').css({ 'position':'absolute', top:'0px', left:'0px' });
				this.canvas = new fabric.Canvas(options.elemID + '-canvas');
				this.canvas.setWidth(options.mapArea.x);
				this.canvas.setHeight(options.mapArea.y);
				//this.canvas.renderAll();
			} else {
				var theERR = 'Js MindMap: Invalid Canvas Renderer';
				if(options.canvasError == 'alert') {
					alert(theERR + ': ' + options.canvasRenderer);
				} else {
					console.error(theERR + ': ' + options.canvasRenderer);
				} //end if else
				throw theERR;
				return;
			} //end if else
			...
			*/
			...
			//-- unixman
			var draw_mindmap = function(elemID) {

				// add the data to the mindmap
				var root = $('#' + elemID + '>ul>li').get(0).mynode = $('#' + elemID).addRootNode($('#' + elemID + '>ul>li>a').text(), {});
				$('#' + elemID + '>ul>li').hide();
				...
				var addLI = function() {
					...
					this.mynode = $('#' + elemID).addNode(parentnode, $('a:eq(0)',this).text(), {
						href: $('a:eq(0)',this).attr('href') ? $('a:eq(0)',this).attr('href') : '#',
