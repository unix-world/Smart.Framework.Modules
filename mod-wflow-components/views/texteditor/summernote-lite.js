...
(function (global, factory) {
	...
	var tableMoveHandler = function (event, col, row) {
		...
		if(typeof event != 'undefined') { // fix by unixman
					...
					dropdown({
						items: [
							...
							//		' data-event="removeFormat" data-value="foreColor">',
									' data-event="foreColor" data-value="inherit">', // fix by unixman
	...
	$$1.extend($$1.summernote.lang, {
		'en-US': {
			...
			video: {
				...
				insert: 'Insert Video',
				url: 'Video URL',
				providers: '(WEBM / OGG / OGV / MP4)', // '(YouTube, Vimeo, Vine, Instagram, DailyMotion or Youku)', // fix by unixman
			},
			link: {
				link: 'Link',
				insert: 'Insert Link',
	...
	var Bullet = /** @class */ (function () {
		...
		Bullet.prototype.indent = function (editable) {
			...
			$$1.each(clustereds, function (idx, paras) {
				...
				if (dom.isLi(head)) {
					...
					if (previousList_1) {
						paras.map(function (para) { return previousList_1.appendChild(para); });
					} else {
						_this.wrapList(paras, head.parentNode.nodeName);
						paras.map(function (para) { return para.parentNode; }).map(function (para) { return _this.appendToPrevious(para); });
					}
				} else {
		...
		Bullet.prototype.findList = function (node) {
				return node ? lists.find(node.children, function (child) { return ['OL', 'UL'].indexOf(child.nodeName) > -1; }) : null;
		};
		/**
	...
	*/
	...
	var Editor = /** @class */ (function () {
		function Editor(context) {
			...
			//	return _this.fontStyling('font-family', "\'" + value + "\'");
				return _this.fontStyling('font-family', "'" + value + "'"); // fix by unixman
			});
	...
	var CodeView = /** @class */ (function () {
		...
		CodeView.prototype.purify = function (value) {
			...
			if (this.options.codeviewFilter) {
				...
				if(!this.options.codeviewIframeAllow) { // fix by unixman
					return value.replace(/(<iframe.*?>.*?(?:<\/iframe>)?)/gi, '');
				} else {
	...
	var Buttons = /** @class */ (function () {
					...
					this.ui.dropdown({
						items: (backColor ? [
							...
							//	'    <button type="button" class="note-color-reset btn btn-light" data-event="removeFormat" data-value="foreColor">',
								'    <button type="button" class="note-color-reset btn btn-light" data-event="foreColor" data-value="inherit">', // fix by unixman
		...
		Buttons.prototype.addImagePopoverButtons = function () {
			...
			this.context.memo('button.floatNone', function () {
				return _this.button({
					contents: _this.ui.icon(_this.options.icons.floatNone),
	...
	var HelpDialog = /** @class */ (function () {
		...
		HelpDialog.prototype.initialize = function () {
			var $container = this.options.dialogsInBody ? this.$body : this.$editor;
			var body = [
				'<div title="SummerNote WYSIWYG Editor 0.8.11" style="background:#FCFCFC;">&nbsp;',
			//	'<p class="text-center">',
			//	'<a href="http://summernote.org/" target="_blank">' +
			//	'SummerNote WYSIWYG Editor 0.8.11' +
			//	'</a> · ',
	...
	$$1.summernote = $$1.extend($$1.summernote, {
		...
		options: {
			...
			fontNames: [
				...
				'Arial', 'Arial Black', 'Courier', 'Courier New', 'Helvetica', 'Lucida Grande',
				'Tahoma', 'Times New Roman', 'Verdana', 'Monaco', 'Palatino'
			],
		...
		//	codeviewFilterRegex: /<\/*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|ilayer|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|t(?:itle|extarea)|xml)[^>]*?>/gi,
			...
			codeviewIframeAllow: false, // improvement by unixman (enable this to allow videos like: youtube, ... that req. iframes)
			codeviewIframeFilter: true,
			codeviewIframeWhitelistSrc: [],
			codeviewIframeWhitelistSrcBase: [ // req. codeviewIframeAllow set to true
