...
class Configuration
{
	...
	private function checkForDeprecatedOptions(array $config): void
	{
		foreach ($config as $key => $value) {
			...
			} elseif ($key === 'italic_style' && $value !== '_') {
			...
			} elseif ($key === 'header_style' && $value !== 'atx') {
				@\trigger_error('Customizing the header_style option is deprecated and may be removed in the next major version', \E_USER_DEPRECATED);
			}
		}
	}
}
