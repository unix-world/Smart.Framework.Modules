...
//======================================================
// Parsedown Markdown (v1) Parser - Output HTML5 Code
// DEPENDS:
//	* Smart::
//	* SmartUnicode::
//	* SmartUtils::
...
//	private const regexTblDefinition 	= '[\t ]*(\{\!DEF\!\=([_A-Za-z0-9\.\-\#;]+)\})[\t ]*'; 							// Table Definition - optional, first head cell starts with: {!DEF!=
	...
	private function prepareHTML(string $markup) {
		...
		//--
		$markup = "\n".'<!--  HTML/PMarkdown :: ( '.Smart::escape_html($info_linebreaks.' '.$info_sbreaks.' '.$info_urls.' '.$info_entities.' '.$info_validatehtml.' T:'.date('YmdHi')).' ) -->'."\n".'<div id="markdown-'.sha1((string)$markup).'-'.Smart::uuid_10_num().'" class="markdown">'."\n".$markup."\n".'</div>'."\n".'<!--  # HTML/PMarkdown # '.Smart::escape_html((string)self::MKDW_VERSION).'  -->'."\n"; // if parsed and contain HTML Tags, add div and comments
		//--
		if($this->validateHtml) {
	...
	private function blockHeader($Line) {
		...
		//	$text = trim($Line['text'], '# ');
			...
			//--
			$attributes = array();
			if(preg_match('/'.self::regexHdrAttribute.'/', $text, $matches)) { // no need for preg_quote() here, self::regexTblCellAttribute is a REGEX expr
	...
	private function blockTable($Line, $Block=null) {
		...
		if((strpos((string)$Block['element']['text'], '|') !== false) AND ((string)rtrim((string)$Line['text'], ' -:|') === '')) {
			...
			//	if(preg_match('/'.self::regexTblDefinition.'/' , (string)$Block['element']['text'], $defs_matches)) { // cannot use because it supports also tables without starting pipe
			...
			foreach($headerCells as $index => $headerCell) {
				...
				//-- unixman
				$matches = array();
				if(preg_match('/'.self::regexTblCellAttribute.'/', $headerCell, $matches)) { // no need for preg_quote() here, self::regexTblCellAttribute is a REGEX expr
		...
		if(($Line['text'][0] === '|') OR (strpos($Line['text'], '|'))) { // here strpos must not check with true/false, because the first character is already checked and must not be checked again
			...
			if(is_array($matches[0])) {
				foreach($matches[0] as $index => $cell) {
					...
					//-- unixman
					$matches = array();
					if(preg_match('/'.self::regexTblCellAttribute.'/', $cell, $matches)) { // no need for preg_quote() here, self::regexTblCellAttribute is a REGEX expr
