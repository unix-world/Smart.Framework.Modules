...
class CoreMacros extends MacroSet
{
	...
	public function macroDump(MacroNode $node, PhpWriter $writer)
	{
		...
		return $writer->write(
			//-- fix by unixman
			'$args = \''.addslashes((string)ltrim((string)$args,'$')).'\'; $arr___debug = (array)get_defined_vars(); if($args) { $arr___debugx = $arr___debug; $arr___debug = []; foreach($arr___debugx as $kk => $vv) { if((string)$kk == (string)$args) { $arr___debug[(string)$kk] = $vv; } } } echo htmlspecialchars(print_r((array)$arr___debug,1));'
